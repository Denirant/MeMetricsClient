...
const MatrixCharts = ({id, name, brand, unit}) => {
    ...
    async function addOnClick(index){
        ...
        try{
            ...
            const {data: res} = await axios.post(`http://109.248.11.166:8080/product/${id}/suggetions/${action}`, item)
    ...
    async function searchItems(){
        try{
            const {data: res} = await axios.post(`http://109.248.11.166:8080/product/${id}/find/suggetions`, { name: name, brand: brand });
