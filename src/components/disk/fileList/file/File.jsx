...
import {deleteFile, downloadFile, getFiles, moveFile} from "../../../../actions/file";
...
import { addSelectedFile, removeSelectedFile, setClickedFile } from '../../../../reducers/fileReducer';
...
const File = ({file}) => {
    ...
    const handleDragStart = (event) => {
        ...
        if(!selected){
            ...
            event.dataTransfer.setData('text/plain', selectedFiles.map(el => el._id).join(','));
        }
    ...
    function handleOpen(e, file){
        ...
        if(!dropdown__container){
            if(
                ...
                alert('File ' + file.name + ' wasa opened!')
            }else{
                alert('File cant be opened!')
            }
        }
    ...
    function handleFileSelect(e){
        ...
        if(!dropdown__container){
            document.getElementById(`file_selector_${file.path}`).checked = !document.getElementById(`file_selector_${file.path}`).checked;
            handleChangeFileSelect(e, document.getElementById(`file_selector_${file.path}`))
        }
    ...
    function moveClickHandler(e){
        ...
        ReactDOM.render(
            <MoveFile prevPath={file.path.split('/').slice(0, -1).join('/')} data={structure} handleCloseForm={() => container.remove()} handleSelectPath={(path) => {
    ...
    const isDraggable = (!selected) ? true : (selectedFiles.includes(file)) ? true : false;

    function getFileIcon(type){
        switch (Object.keys(FileTypes).find(key => FileTypes[key].includes(type.toLowerCase())) || '') {
            case 'image':
                return Image;
            case 'document':
                return Docs;
    ...
    function formatFileName(fileName) {
        ...
        if (fileName.split('.').slice(0, -1).join('.').length > MAX_LENGTH) {
          ...
          return nameWithoutExtension.slice(0, 9) + '...' + fileName.split('.').pop();
        }
    ...
    if (fileView === 'plate') {
        return (
            <div 
                className={`file-plate ${clickedFile === file._id  ? 'file-plate__active' : ''}`} 
            ...
            >
                <div className='file-plate__control'>
                    <img src={getFileIcon(file.type)} alt="" className="file-plate__img"/>
                    {!selected && <Dropdown  
                        imageUrl={dottedMenu}
                        dropContent={
                            [
                ...
                <div className="file_bottom">
                    <div className='file_bottom_info'>
                        ...
                        <div className='file-plate__date'>{moment(file.date).format('DD.MM.YYYY')}</div>
                    </div>
                    {selected && 
                        <div className='file_select'>
                            <input type="checkbox" name='file_selector' id={`file_selector_${file.path}`} checked={selectedFiles.includes(file)} onChange={(e) => handleChangeFileSelect(e, e.target)}/>
