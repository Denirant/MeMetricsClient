...
export const createTodoApi = async (title, start, end) => {
    const data = {
      ...
      try {
        const response = await axios.post('http://109.248.11.166:8080/api/todos', data, {
            headers: {
...
export const getAllTodoApi = async () => {
      ...
      try {
        const response = await axios.get('http://109.248.11.166:8080/api/todos', {
            headers: {
              Authorization: `Bearer ${token}`, // Предполагается, что вы используете токен авторизации
            },
          });
...
export const markTodoAsComplateApi = async (id) => {
    const token = localStorage.getItem('token');
      ...
      try {
        const response = await axios.patch(`http://109.248.11.166:8080/api/todos/${id}/complete`, {}, {
            headers: {
              Authorization: `Bearer ${token}`, // Предполагается, что вы используете токен авторизации
            },
          });
...
export const editTodoApi = async (id, title, start, end) => {
    ...
    const token = localStorage.getItem('token');
      ...
      try {
        const response = await axios.put(`http://109.248.11.166:8080/api/todos/${id}`, data, {
            headers: {
              Authorization: `Bearer ${token}`, // Предполагается, что вы используете токен авторизации
            },
          });    
...
export const deleteTodoApi = async (id) => {
    ...
    try {
        const response = await axios.delete(`http://109.248.11.166:8080/api/todos/${id}`, {
            headers: {
              Authorization: `Bearer ${token}`, // Предполагается, что вы используете токен авторизации
            },
          });
