...
const Signin = ({handleClose, handleSwitch}) => {
    ...
    async function generateCode(id){
        try{	
            const {data: res} = await axios.get('http://109.248.11.166:8080/api/users/OTP', {params: {id: id}});
    ...
    const handleSubmit = async(event) => {
        ...
        try{
            const url = 'http://109.248.11.166:8080/api/users/login';
            ...
            if(error.response && error.response.status >= 400 && error.response.status <= 500){
                ...
                if(error.response.data.message === 'Invalid Email or Password!'){
                    ...
                    try{
                        const url = 'http://109.248.11.166:8080/api/workers/login';
                        const {data: res} = await axios.post(url, data);
